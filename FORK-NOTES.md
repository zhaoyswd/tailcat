...
- **构建号**：`-ldflags` 注入的版本（`v0.6.0-udp.20` 这种）；非发行版构建（伪版本号
  `v0.6.1-0.2026…-dirty`）**掐头留尾**成 `头8~尾11`（长度恒定 ≤ 20）——**不再整串压成 `dev`**：
  `dev` 会把「这个出口跑的是哪个构建」这条诊断信息整个抹掉（2026-09-18 排查时真机上就表现成
  「Build=dev，查不出跑的是哪版」）。
...
### 9. 直连候选地址过滤：虚拟网卡/网段地址不再通告给对端（v0.6.0-udp.20 起）

magicsock 的 `LocalAddrs` 是**本机所有接口**的地址，出口原先把它们**全部**经 CallMeMaybe 通告给对端
当直连候选。于是对端会拿着这些地址每 5s ping 一次，而它们根本拨不通：

```
# 修前的出口日志（8 分钟 320+ 行）
sendto: no route to host   # 目标：192.168.215.0（bridge101）、198.18.0.1（代理 utun）、…
```

现在通告前按三道判据过滤（与 App 线共享同一份实现 `localaddr.go`，两线同源）：

1. **接口名黑名单**（lo / utun / tun / bridge / docker / vmenet / veth / awdl / ancowlan / hw_sate_vnet…）
   ——与出口的物理上行绑定（`--bind-interface`）**共用同一份名单**；
2. **网段地址**：接口地址恰好是自己网段的**网络地址**（手配的 `192.168.215.0/24` 这种；`.0` 不是合法
   主机地址，对端只会得到 `no route to host`）；
3. **自己的隧道地址**（通告它等于让对端往隧道里套隧道）。

判据日志（摘要不变不重复打）：

```bash
grep "advertise: 跳过" ~/tailcat-exit/exit.log | tail -3
# advertise: 跳过 4 条不可达的本机地址（不作为直连候选通告）：192.168.139.3（虚拟接口 bridge100）、192.168.215.0（虚拟接口 bridge101）、198.18.0.1（虚拟接口 utun4）、2001:2:0:6152::1（虚拟接口 utun4）
```

同批收口：UPnP 的内网候选列表（`localIPv4Candidates`）原来只跳过 `lo0/utun/tun`，bridge/docker 地址
照样进候选、每次白吃 5s SSDP 超时；现在复用同一份判据（`upnpIPv4Candidates`，纯函数、排序去重）。

**依赖补丁同批**（`tools/modcache-patches/0001-magicsock-direct-bootstrap.patch`，服务端半边：
同 LAN 优先 / 发送失败降权 / bootstrap 日志节流 / DERP `PeerGone` 后摘掉回退地址）——判据与回归测试见
...
| 版本 | 变化 |
|---|---|---|
| `v0.6.0-udp.21`（2026-09-18） | 修 `v0.6.0-udp.20` 的 **Windows 构建红灯**：发送失败记账的「PMTU 探测错误不计」判据从 `errEMSGSIZE`（只在 unix 版 `peermtu.go` 里定义）换成上游每平台都有定义的 `pmtuShouldLogDiscoTxErr(m, err)` —— udp.20 的 CI 六平台里 `windows/amd64` 编不过 ⇒ release job 被跳过、**没有 Release 产物**，用 udp.21 重发 |
| `v0.6.0-udp.20`（2026-09-18，**已被 udp.21 取代**） | **直连候选策略批次**：候选地址过滤（第 9 节：虚拟网卡/网段地址不再通告、UPnP 候选同源过滤）、构建号超长改「掐头留尾」（不再变 `dev`）、出口侧依赖补丁更新（同 LAN 优先 / 发送失败降权 / bootstrap 日志节流 / DERP 回退摘除） |
| `v0.6.0-udp.19` | 默认项收尾（`--listen-port` 默认 41641、端口占用回退随机并警告、端口自洽过滤用实际绑定端口、`exit-node` 隐式带 `files`） |
