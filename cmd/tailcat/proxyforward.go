...
// dialForward 按需要经代理拨 TCP；未配置代理时用 direct 拨号器（调用方传的是带
// localhostdns 解析器的那个，这样 "localhost" 不依赖操作系统的 hosts 文件）。
func dialForward(ctx context.Context, direct *net.Dialer, addr string) (net.Conn, error) {
	if forwardProxy == nil {
		if direct == nil {
			direct = new(net.Dialer)
		}
