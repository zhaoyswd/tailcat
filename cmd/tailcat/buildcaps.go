...
//
// 能力位是**构建级**声明，不是运行配置：出口带着 agent-gateway 编译但没跑
// files 服务时，地址里照样有 CapAgentGateway。需要按运行配置判断的（例如
// files 服务开没开）由客户端连上去试，见 App 侧的文件错误码。
package main
...
func forkCaps() tailcat.Caps {
	return tailcat.CapUDPForward | tailcat.CapAgentGateway | tailcat.CapFixedPort | tailcat.CapProxyForward
}
