...
func registerExitNodeFlags(rootFS *ff.FlagSet) {
	...
	flagDirectConnect = rootFS.BoolLongDefault("direct-connect", false, `enable direct-first connect (openspec direct-handshake-connect): readiness stops waiting for the relayed meow/meowed round-trip — a TSMP probe races the async meow registration and the first success wins (the server's lazy registration admits WireGuard handshakes before meow lands; on stock servers meow wins the race at no extra cost). Endpoint learning cache participates too. Off by default; the tier app enables it by default.`)
...
// bindInterfaceFlag 供共享的 serve 路径取值（未注册时返回 off）。
func bindInterfaceFlag() string {
	if flagBindInterface == nil {
		return "off"
	}
	v := strings.TrimSpace(*flagBindInterface)
	if v == "" {
		return "off"
	}
	return v
